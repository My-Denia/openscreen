...
| 2026-09-12 | dev build, `fix/509-hud-session-prefs` @ `fbe461e9` plus uncommitted issue-509 patch | Windows 11 | Partial — Settings persistence only | Playwright drove two separate real Electron processes against one isolated userData profile. Recording toggles/device identity and versioned new-project appearance survived a full process restart; a project created before restart and another created after restart both materialized the chosen appearance on their first write. One test passed. This is scripted CDP state/UI evidence and does not count as OS-input capture proof. Native computer-use was unavailable, so source picker interaction, physical microphone/camera, record→edit, missing-source UI behavior, reset through visible controls, tray, export, macOS and Linux were skipped. |
| 2026-09-13 | source-built 1.11.0 integration, settings synchronization repair included | Windows 11, isolated profile | Partial — recording defaults pass | Real OS input verified recording preferences across restart, reset while RecStage remained mounted (previews stop and active source clears), factory defaults on new-project first write, unchanged existing projects and missing-source recovery. The test used the combined tree before PR splitting; it is not a second hardware pass on the split commit. See [recording-defaults-windows.md](recording-defaults-windows.md) for the screenshot, build context and exclusions. No STT, HUD pointer/tray, precise AV sync, other-platform or release-promotion claim. |

| 2026-09-13 | `6862a380` + microphone USB-label resolver patch; rebuilt release helpers | macOS 26.6.2, Apple Silicon, Shure MV7 | Partial | Real desktop HUD: selected explicit `Shure MV7 (14ed:1012)`, microphone on, system audio/camera off; started/stopped capture and reached saved editor (16.1 s). Logs confirm the rebuilt helper path and decorated label. Raw AAC is non-silent (mean -37.4 dBFS, peak -13.3 dBFS). Separately, the resolver matched the decorated label to the real AVFoundation MV7 ID. Default input was also MV7, so audio alone does not prove non-default routing. Listening, mixed/system audio, export, non-default-input recording and other platforms were not run; this is not full acceptance. |
