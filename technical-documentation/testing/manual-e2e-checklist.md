...
- [ ] During GIF rendering, press Cancel and confirm it waits for native cleanup, restores export options, leaves no partial GIF, and preserves an existing destination; retry and confirm the GIF is saved.
...
| 2026-09-13 | source-built 1.11.0 on `fbe461e9` with GIF cancellation, recording defaults and workbench measurement changes | Windows 11, isolated profile | Partial — scoped pass | Real OS input covered screen, system audio, microphone, webcam and combined capture; editor playback, seek and trim; GIF cancel, output cleanup and same-path retry; MP4 export. Media fully decoded. Settings survive restart; reset immediately updates the mounted recording page and stops previews. New projects use saved or reset appearance defaults without altering existing projects. Missing window sources remain unselected after restart. Earlier startup and permission failures were retained in local evidence; the later successful runs are separate. Not covered: STT (helper absent), HUD pointer hit-testing, tray, precise AV sync, exhaustive editor operations, macOS/Linux or release packaging. The existing 15 FPS GIF centisecond timing quantization remains. Private recording evidence is retained locally. |
