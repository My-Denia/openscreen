...
describe("project appearance defaults", () => {
	...
	it("defaults a stored appearance that predates cursor auto-hide", () => {
		const { cursorAutoHide: _cursorAutoHide, cursor, ...rest } = DEFAULT_PROJECT_APPEARANCE;
		const { autoHide: _autoHide, ...legacyCursor } = cursor;
		expect(
			parseProjectAppearanceDefaults({
				...rest,
				cursor: legacyCursor,
			...
			cursor: { ...legacyCursor, autoHide: false },
		});
	});
