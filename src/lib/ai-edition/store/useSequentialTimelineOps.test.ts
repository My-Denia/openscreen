...
describe("useSequentialTimelineOps", () => {
	...
	it("runs a nested enqueue from a sync outer without deadlocking", { timeout: 1000 }, async () => {
		const seed = makeDocWithAsset();
		useProjectStore.setState({ document: seed });
		const saveDocument = vi.fn(async (doc: AxcutDocument) => {
			useProjectStore.getState().setDocument(doc, { history: true });
		...
		const innerEnqueue = () =>
			result.current.enqueue(async () => {
				...
				if (!doc) return false;
				const next = { ...doc, project: { ...doc.project, title: "inner" } };
				return saveDocument(next);
			});

		await act(async () => {
			await result.current.enqueue(() => innerEnqueue());
		});

		expect(useProjectStore.getState().document?.project.title).toBe("inner");
		expect(saveDocument).toHaveBeenCalledTimes(1);
	});

	it("runs a nested enqueue from an async outer that awaits it", { timeout: 1000 }, async () => {
		const seed = makeDocWithAsset();
		useProjectStore.setState({ document: seed });
		const saveDocument = vi.fn(async (doc: AxcutDocument) => {
		...
		const innerEnqueue = () =>
			result.current.enqueue(async () => {
				const doc = useProjectStore.getState().document;
				if (!doc) return false;
				const next = { ...doc, project: { ...doc.project, title: "async-inner" } };
				return saveDocument(next);
			});
		...
		expect(useProjectStore.getState().document?.project.title).toBe("async-inner");
	});

	it("lets a sibling queued with a nested write see the inner commit", {
		timeout: 1000,
	}, async () => {
		const seed = makeDocWithAsset();
		useProjectStore.setState({ document: seed });
		...
		const innerEnqueue = () =>
			result.current.enqueue(async () => {
				const doc = useProjectStore.getState().document;
				if (!doc) return false;
				return saveDocument({ ...doc, project: { ...doc.project, title: "inner" } });
			});

		let siblingTitle = "";
		await act(async () => {
			const nested = result.current.enqueue(() => innerEnqueue());
			const sibling = result.current.enqueue(() => {
				siblingTitle = useProjectStore.getState().document?.project.title ?? "";
			});
			await Promise.all([nested, sibling]);
		});

		expect(siblingTitle).toBe("inner");
	});

	it("does not start a later-turn sibling while an outer save is still blocked", {
		timeout: 1000,
	}, async () => {
		const seed = makeDocWithAsset();
		useProjectStore.setState({ document: seed });
		...
		let siblingSaw = "";
		const outer = result.current.enqueue(async () => {
			const doc = useProjectStore.getState().document;
			if (!doc) return false;
			return saveDocument({ ...doc, project: { ...doc.project, title: "outer" } });
		});
		...
		const sibling = result.current.enqueue(() => {
			siblingStarted = true;
			siblingSaw = useProjectStore.getState().document?.project.title ?? "";
		});
		await act(async () => {
			await Promise.resolve();
		});
