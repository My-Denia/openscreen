 ...
 * `cursor.autoHide` is optional on the editor snapshot (#104); the stored
 * appearance defaults always materialize it as a boolean. */
export type ProjectAppearanceSource = Omit<ProjectAppearanceDefaults, "cursor"> & {
	cursor: Omit<ProjectAppearanceDefaults["cursor"], "autoHide"> & { autoHide?: boolean };
};
...
function cursor(value: unknown): value is ProjectAppearanceDefaults["cursor"] {
	...
	return (
		...
		(candidate.autoHide === undefined || typeof candidate.autoHide === "boolean")
	);
}
...
export function projectAppearanceFromEditorSettings(
	...
	return {
		...
		cursor: { ...settings.cursor, autoHide: settings.cursor.autoHide === true },
		cursorShow: settings.cursorShow,
		cursorAutoHide: settings.cursorAutoHide === true || settings.cursor.autoHide === true,
...
export function applyProjectAppearanceDefaults(
	...
	return {
		...
		legacyEditor: {
			...
			cursorAutoHide: parsed.cursorAutoHide,
